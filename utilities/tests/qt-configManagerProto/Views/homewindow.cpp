...
void HomeWindow::refresh(){
    ...
    foreach(Backup bc, confMan->getUser()->getBackups()){
        ...
        backupList->setDragEnabled(true);
        backupList->setDropIndicatorShown(true);
        backupList->setDragDropMode(QAbstractItemView::InternalMove);
    }
}
...
void HomeWindow::decryptBackup(Backup *bc){
    ...
    const QString source = bc->getTarget()+ QLatin1Char('/')+bc->getName()+ QLatin1String(".vy");
    const QString target = dossier + QLatin1Char('/')+bc->getName();
    if(confMan->decryptDirectory( source, target )){
        emit cryptingDone();
    }
}
...
void HomeWindow::cryptingProgress(quint64 done, quint64 total){
    ...
    cryptProgress->setWindowTitle(QString::number(done) + " / " +QString::number(total) + " fichiers cryptés");
